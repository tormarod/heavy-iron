...
(plans/014). The deploy workflow (`.github/workflows/pages.yml`) guards the
same change against what is live, so if two PRs both bump `CACHE_VERSION`
from the same base, the second deploy fails and requires another bump on
`main`.
...
rather than silently coercing it. A backup or profile file is the app's
