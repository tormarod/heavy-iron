...
| 038 | [One session reader — every read of a logged set goes through `sessionsOf`; one codec owns the row's fields](done/038-session-reader.md) | P2 | L | MED | lb stamp fix (#119, merged) | DONE — all seven PRs (#120, #121, #123, #125, #126, #127 and PR 6); the two accepted visible changes and every deviation are in its Maintenance notes |
...
| 041 | [Four small correctness fixes — a one-set exercise on the deload week, the rep-decay line's "first set", prototype names in the session order, the four `hidden` elements driven by inline display](041-small-fixes-deload-sets-decay-first-set-order-ids-hidden.md) | P2 | S | LOW | — (land after 039 and 040; all three edit `js/app.js`) | BLOCKED at Step D (Steps A, B, C and E landed in PR #133 — the deload clamp, `decayRows` for all three readers, the prototype-safe session order, the header comment; Step D is blocked because `test/smoke.js:1940-1943` waits on `#brakeNote`’s inline `display`, an eighth read the plan’s Current state missed, present since the plan’s own baseline, and `test/smoke.js` is outside the plan’s scope — moving that wait onto the `hidden` attribute is a one-line scope decision for the maintainer, and the dropped Step D edits are the plan’s own text) |
