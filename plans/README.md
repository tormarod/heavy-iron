...
| 047 | [The objetivo's two render memos are keyed by what they depend on; the rule's tests stop leaning on resets](done/047-rule-cache-keys.md) | P3 | S | LOW | — | DONE (#PR047) — both memos keyed by profile; `resetRenderCache()` in unit.js 21 → 4; candidate 3's `targetFrom` split dropped (absorbed by 038 and 045) |
