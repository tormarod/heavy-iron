...
| 023 | [Every imported string in the AI round-trip document is delimited](023-ai-document-delimits-every-imported-string.md) | P1 | S | LOW | — | DONE (`priority.map(reviewName)` is what Step 2 prescribes, so the Done-criteria `grep -c "reviewName("` counts under-read by design; one smoke pin of the old `("Bloque 1")` heading updated per the plan's STOP text — see PR #99) |
| 024 | [Every import rejection path flushes the pending autosave first, never through a two-tab conflict](024-import-preflight-flush-never-through-a-conflict.md) | P1 | S | LOW | — | DONE (a third post-confirm `flushSave()` at the end of `loadProfileFromText` stays, as the plan's contract implies — see PR #100) |
| 025 | [`obj`/`variants` survive every purge, move and import; two v3 rule edges; the dead `'obj'` share entry goes](025-map-integrity-and-v3-edges.md) | P1 | M | LOW–MED | 026 (soft), 021 (soft: Step E) | DONE (Step E skipped — 021 landed it; H1 extends 026's three-rung case rather than duplicating it; Step G's known limit, a unit switch last week, is left as designed — see PR #101) |
...
| 027 | [A set tick reuses the render cache; the Diagnóstico reads through it; `diagPoints` groups once](027-tick-keeps-the-render-cache.md) | P2 | S | MED | 026 (soft) | IN PROGRESS |
