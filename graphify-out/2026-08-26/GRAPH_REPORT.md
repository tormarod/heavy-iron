...
- ~~Are the 2 inferred relationships involving `num()` (e.g. with `.mod()` and `.multiply()`) actually correct?~~ **Resolved: no, both were false positives.**
  _Both edges came from `js/vendor/qrcode.js`'s `QRPolynomial.mod()`/`.multiply()`, which declare a local variable also named `num` — an identifier-name collision with the unrelated `num()` helper in `js/app.js:29`, not a real cross-file call. Removed from graph.json on 2026-08-26._
