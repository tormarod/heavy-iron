...
statistics behind Diagnóstico. It also boots the app in Node (`bootApp()` in
`test/harness.js`) and presses the real handlers, asserting what they write.
...
| `test/harness.js` | what `test/unit.js` loads the app with: `loadApp()` (scripts only, for pure logic) and `bootApp()` (the real `load()` on a remembering fake document and a fake clock, for handler-level data tests) — and the worker with: `loadWorker()` (the real `sw.js` over a fake cache store and network; two sharing one store are two releases side by side) |
| `test/unit.js` | headless assertions for pure logic, through `bootApp()` the real handlers' data effects, and through `loadWorker()` the real `sw.js`'s install, activate, fetch and swap — no server, no browser |
...
- `ex.id`: optional, but **keep it stable** — the weekly objetivo joins
...
- `phase`: optional — per-week (`1`–`weeks`) goal text shown in the
  banner. An entry needs both `r` and `t`; any week left out, or with only
  one of the two, falls back to the generic RIR-based default.
...
| `ex.minRir` | rounded to a whole number and clamped to 1–5; `0` or anything unreadable drops the field |
